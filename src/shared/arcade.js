import { EXTRA_GAMES, initExtra, extraAction, extraDirection, updateExtra } from './extra-games.js';
import { ANTHOLOGY_GAMES } from './anthology/catalog.js';
import { initDiscovery, discoveryAction, discoveryDirection, updateDiscovery } from './anthology/engine.js';
import { LEVELS, MAX_ARCADE_LEVEL } from './arcade-levels.js';
export { LEVELS, MAX_ARCADE_LEVEL };
...
export function createArcade(kind, level=1, rng=Math.random) {
  ...
  const gameId=kind, rules=ARCADE_GAMES[kind];
  kind=rules.base ?? kind;
  const campaignLevel=clamp(Math.floor(Number(level)||1),1,MAX_ARCADE_LEVEL);
  level=Math.min(campaignLevel,5);
  const s={kind,level:campaignLevel,difficulty:level,time:0,score:0,lives:3,over:false,won:false,x:450,y:450,spawn:0,items:[],shots:[],cooldown:0,invincible:0,target:10+level*3};
  if(rules.anthology) { s.kind='discovery'; s.gameId=gameId; s.speed=1; initDiscovery(s,rules,rng); return s; }
  ...
  if(rules.base) {
    ...
    if(!['bricks','dodge','race'].includes(kind)) s.target=Math.max(1,Math.round(s.target*rules.distance));
    if(kind==='bricks') {
      // Longer challenges require more hits per block, shorter ones use fewer blocks.
      if(rules.distance<1) s.bricks=s.bricks.filter((_,i)=>i%5<Math.ceil(5*rules.distance));
      else if(rules.distance>1) for(const b of s.bricks)b.hp+=Math.round((rules.distance-1)*2);
      s.target=s.bricks.length;
    }
  }
  return s;
...
function newTarget(s,rng) {s.mark={x:70+rng()*760,y:80+rng()*370,r:38-s.difficulty*3,vx:(rng()<0.5?-1:1)*(35+s.difficulty*18)};}
function makeMaze(s,rng) {
  const n=9+s.difficulty*2;s.n=n;s.grid=Array.from({length:n},()=>Array(n).fill(1));
  ...
  s.keys=shuffle(free,rng).slice(0,s.difficulty);s.target=s.keys.length;
}
...
export function arcadeAction(s,x,y,rng=Math.random) {
  if(s.over)return;
  if(s.kind==='discovery'){discoveryAction(s,x,y);return;}
  if(Object.hasOwn(EXTRA_GAMES,s.kind)){extraAction(s,x,y,rng);return;}
  x ??= s.x;y ??= s.y;
...
export function arcadeDirection(s,dx,dy) {
  if(s.kind==='discovery'){discoveryDirection(s,dx,dy);return;}
  if(Object.hasOwn(EXTRA_GAMES,s.kind)){extraDirection(s,dx,dy);return;}
  if(s.kind==='snake' && (dx||dy) && !(dx===-s.dir.x&&dy===-s.dir.y)) s.pending={x:dx,y:dy};
}
export function updateArcade(s,dt,input={},rng=Math.random) {
  if(s.over)return;
  dt=clamp(dt,0,0.04)*(s.speed ?? 1);s.time+=dt;s.cooldown-=dt;s.invincible=Math.max(0,s.invincible-dt);
  if(s.kind==='discovery'){updateDiscovery(s,dt,input);return;}
  if(Object.hasOwn(EXTRA_GAMES,s.kind)){updateExtra(s,dt,input,rng);return;}
  ...
  if(s.kind==='snake') {
    s.clock+=dt;if(s.clock>=0.23-s.difficulty*0.025){s.clock=0;s.dir={...s.pending};const head={x:s.body[0].x+s.dir.x,y:s.body[0].y+s.dir.y},eat=head.x===s.food.x&&head.y===s.food.y;
  ...
  if(s.kind==='mole') {
    ...
    s.spawn-=dt;if(s.spawn<=0){s.spawn=0.85-s.difficulty*0.09;const empty=s.holes.map((h,i)=>h.time<=0?i:-1).filter(i=>i>=0);if(empty.length)s.holes[empty[pick(rng,empty.length)]]={time:1.35-s.difficulty*0.12,bad:rng()<0.12+s.difficulty*0.025};}
  }
  ...
  s.spawn-=dt;
  if(s.kind==='stars') {
    if(s.spawn<=0){s.spawn=0.65-s.difficulty*0.055;s.items.push({x:30+rng()*840,y:-20,bad:rng()<0.18+s.difficulty*0.03});}
    for(const p of s.items){p.y+=(170+s.difficulty*35)*dt;if(p.y>475&&p.y<520&&Math.abs(p.x-s.x)<65){if(p.bad)hurt(s);else s.score++;p.y=600;}}
    s.items=s.items.filter(p=>p.y<570);
  }
  if(s.kind==='runner') {
    s.vy+=1500*dt;s.y=Math.min(440,s.y+s.vy*dt);if(s.y===440)s.vy=0;
    if(s.spawn<=0){s.spawn=1.65-s.difficulty*0.1+rng()*0.35;s.items.push({x:940,w:26+rng()*20,h:35+rng()*25});}
    for(const p of s.items){p.x-=(260+s.difficulty*35)*dt;if(p.x<192&&p.x+p.w>148&&s.y>450-p.h){hurt(s);p.hit=true;}
      if(p.x+p.w<140&&!p.passed){p.passed=true;if(!p.hit)s.score++;}}
    s.items=s.items.filter(p=>p.x>-60);
  }
  if(s.kind==='dodge') {
    if(s.spawn<=0){s.spawn=0.42-s.difficulty*0.035;s.items.push({x:25+rng()*850,y:-30,r:14+rng()*15,vx:(rng()-0.5)*s.difficulty*25});}
    for(const p of s.items){p.y+=(160+s.difficulty*35)*dt;p.x+=p.vx*dt;if(near(p,s,p.r+17)){hurt(s);p.y=700;}}
    s.items=s.items.filter(p=>p.y<590);if(s.time>=s.limit)finish(s,true);
  }
  if(s.kind==='shooter') {
    if(s.spawn<=0){s.spawn=1.0-s.difficulty*0.1;s.items.push({x:40+rng()*820,y:-20});}
    ...
    for(const p of s.items){p.y+=(80+s.difficulty*25)*dt;if(!p.dead&&(p.y>535||near(p,{x:s.x,y:485},32))){hurt(s);p.dead=true;}}
  ...
  if(s.kind==='flappy') {
    ...
    for(const p of s.items){p.x-=(170+s.difficulty*22)*dt;if(p.x<s.x+16&&p.x+65>s.x-16&&(s.y-14<p.gap-half||s.y+14>p.gap+half)){finish(s,false);return;}if(p.x+65<s.x&&!p.passed){s.score++;p.passed=true;}}
    if(s.y<14||s.y>536)finish(s,false);s.items=s.items.filter(p=>p.x>-80);
  }
  if(s.kind==='race') {
    s.x=clamp(s.x,285,615);
    if(s.spawn<=0){s.spawn=1.2-s.difficulty*0.1;s.items.push({x:[310,450,590][pick(rng,3)],y:-90});}
    for(const p of s.items){p.y+=(230+s.difficulty*45)*dt;if(Math.abs(p.x-s.x)<49&&Math.abs(p.y-455)<70){hurt(s);p.y=700;}}
...
function updateBall(s,dt){
  ...
  if(s.kind==='pong'){
    const goal=clamp(s.by,55,495),speed=155+s.difficulty*30;
    ...
    if(s.by>570){s.lives--;s.bx=s.x;s.by=440;s.vy=-230-s.difficulty*20;s.serve=0.7;}
