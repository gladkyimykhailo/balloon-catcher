...
export function drawArcade(c,s,overlay='') {
  const [bg,mid,accent]=PALETTES[s.level-1];
  ...
  if(s.kind==='bricks') {
    const half=70-s.level*4;rect(s.x-half,490,half*2,18,accent);
    for(const b of s.bricks)if(b.hp>0){rect(b.x,b.y,78,24,b.hp===2?'#fff0ad':['#ffb45c','#fa7297','#75d6af','#83afff'][Math.floor((b.y-45)/29)%4]);if(b.hp===2)rect(b.x+8,b.y+5,62,3,'#b88342',1);}
  }
  ...
  if(s.kind==='race') {
    ...
    for(const x of [378,518])for(let y=-80;y<550;y+=95)rect(x,(y+(s.time*(230+s.level*45))%95),4,45,'#ffffff70',2);
  ...
  if(overlay){rect(0,0,900,550,'#081324bb',0);rect(145,180,610,170,bg,25);text(overlay,450,245,32);text(s.over?(s.won?'Обирай наступний рівень!':'Натисни «Заново», щоб спробувати ще'):'Натисни «Грати», коли будеш готовий',450,300,20,accent);}
}
